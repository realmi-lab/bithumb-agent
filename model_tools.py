...
# Bithumb Agent contains no plugin tool discovery. Only the coding modules
# admitted by ``discover_builtin_tools`` above can register handlers.
...
def get_tool_definitions(
    ...
    """
    from hermes_cli.bithumb_agent_policy import restrict_toolsets

    # Fail closed at the schema boundary.  Upstream callers that pass None,
    # ``all``, plugin toolsets, or MCP aliases cannot widen Bithumb Agent beyond its
    # reviewed coding allow-list.
    enabled_toolsets = restrict_toolsets(enabled_toolsets)
...
def handle_function_call(
    ...
    """
    from hermes_cli.bithumb_agent_policy import tool_is_allowed

    # Schema filtering is not a sufficient security boundary: a malformed or
    # compromised caller could dispatch a registered tool by name.  Enforce
    # the same coding-only allow-list immediately before all bridge, hook, and
    # registry execution paths.
    if not tool_is_allowed(function_name):
        return json.dumps(
            {"error": f"Tool {function_name!r} is disabled by Bithumb Agent security policy."},
            ensure_ascii=False,
        )
