...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    ...
    # Bithumb Agent is a coding-only distribution.  Do not even import unrelated
    # built-in tool modules: several of them initialize network clients or
    # browser/provider discovery as import-time dependencies.  This is a
    # fail-closed allow-list, so a newly added upstream tool stays unavailable
    # until it is deliberately reviewed and admitted here.
    try:
        from hermes_cli.bithumb_agent_policy import ALLOWED_BUILTIN_TOOL_MODULES

        module_names = [
            name for name in module_names if name in ALLOWED_BUILTIN_TOOL_MODULES
        ]
    except ImportError:
        module_names = []
