#!/usr/bin/env python3
"""Local terminal tool for Bithumb Agent.

Only the host-local execution backend is shipped. Remote SSH, cloud sandbox,
managed gateway, and container-provider implementations are intentionally
absent from this distribution.
...
# Bithumb Agent ships only the local environment implementation.
...
def _get_env_config() -> Dict[str, Any]:
    """Return the non-overridable local execution configuration."""

    cwd = os.path.abspath(os.path.expanduser(os.getenv("TERMINAL_CWD") or _safe_getcwd()))
    return {
        "env_type": "local",
        "cwd": cwd,
...
def _create_environment(
    ...
    """Create the only supported execution backend: the local workspace."""

    del image, ssh_config, container_config, local_config, task_id, host_cwd
    if env_type != "local":
        raise ValueError("Bithumb Agent only supports the local terminal backend")
...
def check_terminal_requirements() -> bool:
    """The local backend is always available when Python is running."""
