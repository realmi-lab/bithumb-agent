...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    if agent.provider == "copilot-acp" or str(client_kwargs.get("base_url", "")).startswith("acp://copilot"):
        ...
        return client
    if agent.provider == "antigravity-cli" or str(client_kwargs.get("base_url", "")).startswith("antigravity-cli://"):
        from agent.antigravity_cli_client import AntigravityCLIClient

        client = AntigravityCLIClient(**client_kwargs)
        _ra().logger.info(
            "Antigravity CLI OAuth client created (%s, shared=%s) %s",
            reason,
            shared,
            agent._client_log_context(),
        )
