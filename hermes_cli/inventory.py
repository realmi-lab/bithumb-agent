...
def build_models_payload(
    ...
    *,
    ...
    # Bithumb Agent exposes concrete subscription OAuth backends only. The upstream
    # virtual MoA provider can reference API-key aggregators, so it is not part
    # of this fork's picker inventory.
    from hermes_cli.bithumb_agent_policy import ALLOWED_PROVIDER_SET
    rows = [
        row
        for row in rows
        if str(row.get("slug", "")).strip().lower() in ALLOWED_PROVIDER_SET
    ]
