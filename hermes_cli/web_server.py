...
async def update_hermes():
    ...
    if os.environ.get("BITHUMB_AGENT_DISABLE_AUTO_UPDATE") == "1":
        message = "Bithumb Agent self-update is disabled by the security policy."
        _record_completed_action("hermes-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "hermes-update",
            "error": "bithumb_agent_update_disabled",
            "message": message,
            "update_command": "review and deploy a pinned build manually",
        }
...
async def check_hermes_update(force: bool = False):
    ...
    if os.environ.get("BITHUMB_AGENT_DISABLE_AUTO_UPDATE") == "1":
        return {
            "install_method": "pinned-bithumb-agent",
            "current_version": __version__,
            "behind": None,
            "update_available": False,
            "can_apply": False,
            "update_command": "review and deploy a pinned build manually",
            "message": "Bithumb Agent update checks are disabled by security policy.",
        }
...
def _is_bithumb_agent_inference_credential_key(key: str) -> bool:
    """Return whether an env key belongs to a disabled LLM credential path."""
    normalized = str(key or "").strip()
    if not normalized:
        return False
    optional = OPTIONAL_ENV_VARS.get(normalized) or {}
    if optional.get("category") == "provider":
        return True
    return normalized in _catalog_provider_env_metadata()
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    for var_name, info in OPTIONAL_ENV_VARS.items():
        if _is_bithumb_agent_inference_credential_key(var_name):
            continue
...
async def set_env_var(body: EnvVarUpdate, profile: Optional[str] = None):
    if _is_bithumb_agent_inference_credential_key(body.key):
        raise HTTPException(
            status_code=400,
            detail="Bithumb Agent inference is OAuth-only; provider API keys are disabled.",
        )
    try:
        with _profile_scope(body.profile or profile):
...
def list_custom_endpoints():
    ...
    return {"endpoints": [], "current": {"provider": "", "model": "", "base_url": ""}}
...
def upsert_custom_endpoint(body: CustomEndpointUpdate):
    ...
    raise HTTPException(
        status_code=400,
        detail="Bithumb Agent inference is OAuth-only; custom endpoints are disabled.",
    )

    # Retained upstream implementation below for source-update comparison.
    try:
        cfg = load_config()
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "ChatGPT OAuth",
        "flow": "device_code",
        "cli_command": "bithumb-agent auth add openai-codex",
        "docs_url": "https://chatgpt.com/",
        "status_fn": None,  # dispatched via auth.get_codex_auth_status
    },
    {
        "id": "antigravity-cli",
        "name": "Gemini via Antigravity OAuth",
        "flow": "external",
        "cli_command": "bithumb-agent auth add antigravity-cli",
        "docs_url": "https://antigravity.google/",
        "status_fn": None,  # dispatched via auth.get_auth_status
    },
)
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        ...
        if provider_id == "antigravity-cli":
            raw = hauth.get_auth_status("antigravity-cli")
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "official_antigravity_cli",
                "source_label": raw.get("resolved_command") or "Official Antigravity CLI",
                "token_preview": None,
                "expires_at": None,
                "has_refresh_token": False,
                "overage_protected": bool(raw.get("overage_protected")),
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    ``hermes model`` order.
    """
    # Do not union the upstream plugin/provider catalog here. Bithumb Agent has an
    # intentionally closed provider surface and its runtime rejects everything
    # outside this two-entry list as well.
    return [dict(entry) for entry in _OAUTH_PROVIDER_CATALOG]
...
async def list_credential_pool():
    ...
    from hermes_cli.bithumb_agent_policy import ALLOWED_PROVIDER_SET

    for provider_id in sorted(raw_pool.keys()):
        if provider_id not in ALLOWED_PROVIDER_SET:
            continue
        try:
...
async def add_credential_pool_entry(body: CredentialPoolAdd):
    raise HTTPException(
        status_code=400,
        detail="Bithumb Agent inference is OAuth-only; manual API-key pools are disabled.",
    )
...
def _profile_scope(profile: Optional[str]):
    ...
    # Bithumb Agent's coding-only distribution does not ship the upstream
    # skills-management tools. Model/auth endpoints still reuse this context
    # manager for profile-local config, so keep that scope functional when the
    # optional modules are intentionally absent.
    try:
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Bithumb Agent Teal",         "description": "Bithumb Agent dark teal"},
    {"name": "default-large", "label": "Bithumb Agent Teal (Large)", "description": "Bithumb Agent Teal with bigger fonts and roomier spacing"},
