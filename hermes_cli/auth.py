...
DEFAULT_ANTIGRAVITY_CLI_BASE_URL = "antigravity-cli://local"
...
PROVIDER_REGISTRY: Dict[str, ProviderConfig] = {
    ...
    "antigravity-cli": ProviderConfig(
        ...
        base_url_env_var="BITHUMB_AGENT_ANTIGRAVITY_CLI_BASE_URL",
    ),
...
def resolve_provider(
    ...
    *,
    ...
    from hermes_cli.bithumb_agent_policy import (
        ...
        normalize_provider as normalize_bithumb_agent_provider,
        reject_api_credentials,
    )

    try:
        reject_api_credentials(api_key=explicit_api_key, base_url=explicit_base_url)
        normalized = normalize_bithumb_agent_provider(requested, allow_auto=True)
    except BithumbAgentProviderError as exc:
        raise AuthError(str(exc), code="bithumb_agent_oauth_only") from exc

    if normalized != "auto":
        return normalized

    # Bithumb Agent' auto mode is intentionally small and deterministic. It never
    # scans API-key environment variables or custom endpoints.
    try:
        from hermes_cli.config import load_config

        model_cfg = (load_config() or {}).get("model")
        if isinstance(model_cfg, dict):
            configured = model_cfg.get("provider")
            if isinstance(configured, str) and configured.strip():
                try:
                    return normalize_bithumb_agent_provider(configured, allow_auto=False)
                except BithumbAgentProviderError:
                    pass
    except Exception:
        pass
    ...
    try:
        ...
        if active:
            active = normalize_bithumb_agent_provider(active, allow_auto=False)
    ...
    for candidate in ("openai-codex", "antigravity-cli"):
        try:
            status = get_auth_status(candidate)
            if status.get("logged_in"):
                return candidate
        except Exception:
            continue
    ...
    raise AuthError(
        "No Bithumb Agent OAuth login is ready. Run `bithumb-agent auth add openai-codex` "
        "or install Antigravity CLI and run `bithumb-agent auth add antigravity-cli`.",
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "antigravity-cli":
        ...
        command = resolve_antigravity_command()
        args = resolve_antigravity_args()
    else:
        command = (
    ...
    if provider_id == "antigravity-cli":
        resolved_command = resolve_antigravity_executable(command)
    else:
        resolved_command = shutil.which(command) if command else None
    cli_installed = bool(resolved_command or base_url.startswith("acp+tcp://"))
    auth_error = ""
    if provider_id == "antigravity-cli":
        # The OAuth token lives in the OS keyring. Probe with Antigravity's
        # quota command, which the official CLI handles without spending model
        # quota; never inspect or copy the token itself.
        oauth_path = None
        if cli_installed:
            logged_in, auth_error = probe_antigravity_login(
                resolved_command or command,
                args,
            )
        else:
            logged_in = False
        overage_protected = credit_overages_are_disabled()
    else:
        oauth_path = None
        logged_in = cli_installed
        overage_protected = False
    return {
        "configured": bool(cli_installed and logged_in),
        "provider": provider_id,
        "name": pconfig.name,
        "command": command,
        "args": args,
        ...
        "oauth_store": str(oauth_path) if oauth_path is not None else "",
        "login_managed_by_cli": provider_id == "antigravity-cli",
        "overage_protected": overage_protected,
        "error": auth_error,
    }
...
def get_auth_status(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if target in {"copilot-acp", "antigravity-cli"}:
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        if provider_id == "antigravity-cli":
            raise AuthError(
                f"Could not find the Antigravity CLI command '{command}'. Install "
                "the official `agy` CLI, then run `bithumb-agent auth add antigravity-cli`.",
                provider=provider_id,
    ...
    if provider_id == "antigravity-cli":
        # Fail closed against paid overages and API-key mode every time this
        # runtime is selected. Authentication itself remains owned by agy.
        enforce_oauth_without_credit_overages()

    return {
        "provider": provider_id,
        # Marker only; no provider API key is stored or sent.
        "api_key": provider_id,
        "base_url": base_url.rstrip("/"),
