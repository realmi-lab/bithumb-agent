"""Bithumb Agent welcome banner, ASCII art, skills summary, and update check.

The runtime remains Hermes-compatible; this module owns the downstream visual brand.
"""
import json
import logging
import os
...
BITHUMB_AGENT_LOGO = """[bold #FF8A3D]██████╗ ██╗████████╗██╗  ██╗██╗   ██╗███╗   ███╗██████╗        █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FF8A3D]██╔══██╗██║╚══██╔══╝██║  ██║██║   ██║████╗ ████║██╔══██╗      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#F37021]██████╔╝██║   ██║   ███████║██║   ██║██╔████╔██║██████╔╝█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#F37021]██╔══██╗██║   ██║   ██╔══██║██║   ██║██║╚██╔╝██║██╔══██╗╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CF4F08]██████╔╝██║   ██║   ██║  ██║╚██████╔╝██║ ╚═╝ ██║██████╔╝      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
BITHUMB_MONOGRAM = """[bold #FF8A3D]⠀⠀⠀⠀⠀⠀⠀⠀██████╗⠀⠀⠀⠀⠀⠀⠀⠀[/]
[bold #FF8A3D]⠀⠀⠀⠀⠀⠀⠀⠀██╔══██╗⠀⠀⠀⠀⠀⠀⠀[/]
[#F37021]⠀⠀⠀⠀⠀⠀⠀⠀██████╔╝⠀⠀⠀⠀⠀⠀⠀[/]
[#F37021]⠀⠀⠀⠀⠀⠀⠀⠀██╔══██╗⠀⠀⠀⠀⠀⠀⠀[/]
[#CF4F08]⠀⠀⠀⠀⠀⠀⠀⠀██████╔╝⠀⠀⠀⠀⠀⠀⠀[/]
[#CF4F08]⠀⠀⠀⠀⠀⠀⠀⠀╚═════╝⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#FF9F63]⠀⠀⠀⠀⠀BUILD · VERIFY · SHIP⠀⠀⠀⠀⠀[/]
[dim #A54100]⠀⠀⠀⠀open-source coding agent⠀⠀⠀⠀[/]"""

# Backward-compatible names used by the upstream Hermes display pipeline.
...
def format_banner_version_label() -> str:
    ...
    base = f"Bithumb Agent v{VERSION} ({RELEASE_DATE}) · OSS customization"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    accent = _skin_color("banner_accent", "#F37021")
    dim = _skin_color("banner_dim", "#A54100")
    text = _skin_color("banner_text", "#FFF3EB")
    ...
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Bithumb Agent[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Bithumb Agent[/]")

    left_lines.append(f"[bold {accent}]OPEN-SOURCE CUSTOMIZATION[/]")
    left_lines.append(f"[bold {text}]오픈소스를 빗썸에 맞게 바꾼 것입니다.[/]")
    left_lines.append(f"[bold {text}]ilhong.kim@bithumbcorp.com[/]")
    left_lines.append(f"[dim {dim}]Hermes Agent (MIT) core · Antigravity-inspired UX[/]")
    left_lines.append(f"[dim {dim}]Independent project · upstream notices preserved[/]")
    ...
    title_color = _skin_color("banner_title", "#FF8A3D")
    border_color = _skin_color("banner_border", "#CF4F08")
