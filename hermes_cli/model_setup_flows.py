...
def _model_flow_antigravity_cli(config, current_model=""):
    """Select Gemini through the official Antigravity Google OAuth session."""
    from hermes_cli.auth import (
    ...
    status = get_external_process_provider_status("antigravity-cli")
    if not status.get("cli_installed"):
        print("  Antigravity CLI is not installed.")
        print("  Install: curl -fsSL https://antigravity.google/cli/install.sh | bash")
        print("  Login:   bithumb-agent auth add antigravity-cli")
        print("  Then run `hermes model` again.")
        return
    if not status.get("logged_in"):
        print("  Antigravity CLI is installed, but Google OAuth is not connected.")
        print("  Run: bithumb-agent auth add antigravity-cli")
        print("  Then run `hermes model` again.")
        return

    command = status.get("resolved_command") or status.get("command") or "agy"
    print(f"  Antigravity CLI: {command}")
    print("  Authentication is managed by the official Antigravity Google login.")
    print("  Bithumb Agent does not read, copy, or store the Google OAuth token.")
    print("  Paid AI-credit overages: disabled")
    _update_config_for_provider(
        "antigravity-cli",
        DEFAULT_ANTIGRAVITY_CLI_BASE_URL,
        default_model="auto",
    )
    print("Default model set to: auto (via Antigravity OAuth)")
