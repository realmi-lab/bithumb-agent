...
def list_authenticated_providers(
    ...
    *,
    ...
    endpoint.
    """
    # Bithumb Agent deliberately replaces the upstream API-key/custom-endpoint
    # discovery graph with two subscription OAuth providers. Keeping the
    # upstream implementation below makes future source updates reviewable,
    # while this early policy boundary prevents a configured ambient API key
    # from leaking a disabled provider into any CLI, TUI, or dashboard picker.
    from hermes_cli.bithumb_agent_policy import ALLOWED_PROVIDERS
    from hermes_cli.auth import get_auth_status
    from hermes_cli.models import (
        _PROVIDER_MODELS as _bithumb_agent_model_catalog,
        cached_provider_model_ids as _bithumb_agent_cached_model_ids,
        clear_provider_models_cache as _bithumb_agent_clear_model_cache,
    )

    if refresh:
        try:
            _bithumb_agent_clear_model_cache()
        except Exception:
            pass
    ...
    for provider_id in ALLOWED_PROVIDERS:
        ...
        try:
            model_ids = list(_bithumb_agent_cached_model_ids(provider_id))
        except Exception:
            model_ids = list(_bithumb_agent_model_catalog.get(provider_id, []))
        if not model_ids:
            model_ids = list(_bithumb_agent_model_catalog.get(provider_id, []))
        if current_model and current == provider_id and current_model not in model_ids:
            model_ids.insert(0, current_model)

        total = len(model_ids)
        visible_models = model_ids[:max_models] if max_models is not None else model_ids
        rows.append({
            "slug": provider_id,
            "name": labels[provider_id],
            "is_current": provider_id == current,
            "is_user_defined": False,
            "models": visible_models,
            "total_models": total,
            "source": "bithumb-agent-oauth",
        })

    rows.sort(key=lambda row: (not row["is_current"], ALLOWED_PROVIDERS.index(row["slug"])))
    return rows
