...
_OAUTH_CAPABLE_PROVIDERS = {"openai-codex", "antigravity-cli"}
...
def _normalize_provider(provider: str) -> str:
    from hermes_cli.bithumb_agent_policy import normalize_provider

    return normalize_provider(provider, allow_auto=False)
...
def auth_add_command(args) -> None:
    ...
    if requested_type == AUTH_TYPE_API_KEY:
        raise SystemExit(
            "Bithumb Agent is OAuth-only. API-key credentials cannot be added."
        )
    ...
    if provider == "antigravity-cli":
        ...
        command = status.get("resolved_command") or status.get("command") or "agy"
        if not status.get("cli_installed") and not shutil.which(str(command)):
            raise SystemExit(
                "Antigravity CLI is not installed. Install it from "
                "https://antigravity.google/docs/cli/install/ and retry "
                "`bithumb-agent auth add antigravity-cli`."
            )
        settings_path = enforce_oauth_without_credit_overages()
        print("Opening Antigravity CLI. Sign in with Google, then type `/exit` when ready.")
        print("Paid AI-credit overages are disabled for Bithumb Agent.")
        ...
        if completed.returncode != 0:
            raise SystemExit(f"Antigravity CLI exited with status {completed.returncode}.")
        enforce_oauth_without_credit_overages()
        refreshed = auth_mod.get_external_process_provider_status(provider)
        if not refreshed.get("logged_in"):
            reason = str(refreshed.get("error") or "authentication was not detected").strip()
            raise SystemExit(
                "Antigravity CLI closed without a verified Google OAuth login: " + reason
            )
        auth_mod._update_config_for_provider(
            provider,
            auth_mod.DEFAULT_ANTIGRAVITY_CLI_BASE_URL,
            "auto",
        )
        print("Antigravity OAuth is ready. Bithumb Agent did not copy or store a Google token.")
        print(f"No-overage setting: {settings_path}")
        return
...
def auth_list_command(args) -> None:
    raw_filter = getattr(args, "provider", "") or ""
    try:
        provider_filter = _normalize_provider(raw_filter) if raw_filter else ""
    except ValueError as exc:
        raise SystemExit(str(exc)) from exc
    if provider_filter:
        providers = [provider_filter]
    else:
        providers = ["openai-codex", "antigravity-cli"]
    for provider in providers:
        if provider == "antigravity-cli":
            status = auth_mod.get_external_process_provider_status(provider)
            if not status.get("cli_installed"):
                state = "CLI not installed"
            elif status.get("logged_in"):
                if status.get("overage_protected"):
                    state = "Google OAuth connected; paid overages disabled"
                else:
                    state = "Google OAuth connected; overage protection needs repair"
            else:
                state = "CLI installed; Google OAuth login required"
            print(f"antigravity-cli: {state}")
            continue
        pool = load_pool(provider)
        entries = pool.entries()
        if not entries:
            status = auth_mod.get_auth_status(provider)
            state = "OAuth connected" if status.get("logged_in") else "OAuth login required"
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required: openai-codex or antigravity-cli.")
    status = auth_mod.get_auth_status(provider)
...
def auth_logout_command(args) -> None:
    provider = _normalize_provider(getattr(args, "provider", None))
    if provider == "antigravity-cli":
        print("Gemini OAuth is owned by the official Antigravity CLI, not Bithumb Agent.")
        print("Run `agy` and use `/logout` to remove the Google login.")
        return
...
def _interactive_auth() -> None:
    """Show Bithumb Agent' two OAuth login paths when called without a subcommand."""
    print("Bithumb Agent OAuth Status")
    print("=" * 50)

    auth_list_command(SimpleNamespace(provider=None))
    print()
    print("Connect ChatGPT:  bithumb-agent auth add openai-codex")
    print("Connect Gemini:   bithumb-agent auth add antigravity-cli")
    return
