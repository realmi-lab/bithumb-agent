...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool install --force git+https://github.com/realmi-lab/bithumb-agent.git"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade git+https://github.com/realmi-lab/bithumb-agent.git"
        return "pip install --upgrade git+https://github.com/realmi-lab/bithumb-agent.git"
    return "bithumb-agent update"
...
def ensure_hermes_home():
    ...
    # Bithumb Agent does not use Hermes' cron, memory, pairing, hook, media-cache,
    # skill, or persona surfaces.  Avoid recreating those directories and
    # SOUL.md on every config read after the operator removes them.
    if os.environ.get("HERMES_SAFE_MODE") == "1":
        home.mkdir(parents=True, exist_ok=True)
        _secure_dir(home)
        logs_dir = home / "logs"
        logs_dir.mkdir(parents=True, exist_ok=True)
        _secure_dir(logs_dir)
