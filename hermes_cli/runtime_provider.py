...
from hermes_cli.auth import (
    ...
    DEFAULT_ANTIGRAVITY_CLI_BASE_URL,
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    from hermes_cli.bithumb_agent_policy import BithumbAgentProviderError, normalize_provider

    if requested and requested.strip():
        try:
            return normalize_provider(requested, allow_auto=True)
    ...
    if isinstance(cfg_provider, str) and cfg_provider.strip():
        try:
            return normalize_provider(cfg_provider, allow_auto=True)
        except BithumbAgentProviderError as exc:
            raise AuthError(
                f"{exc} Update model.provider with `hermes model`.",
                code="bithumb_agent_oauth_only",
            ) from exc
    ...
    if env_provider:
        try:
            return normalize_provider(env_provider, allow_auto=True)
        except BithumbAgentProviderError as exc:
            raise AuthError(str(exc), code="bithumb_agent_oauth_only") from exc
...
def resolve_runtime_provider(
    *,
    ...
    """
    from hermes_cli.bithumb_agent_policy import BithumbAgentProviderError, reject_api_credentials
    ...
    # Bithumb Agent stops here: resolve exactly one of the two OAuth-backed runtimes
    # before any upstream custom endpoint, credential pool, API-key env, or
    ...
    if provider == "antigravity-cli":
        ...
        return {
            ...
            "api_mode": "chat_completions",
            "base_url": creds.get("base_url", "").rstrip("/") or DEFAULT_ANTIGRAVITY_CLI_BASE_URL,
            "api_key": creds.get("api_key", "antigravity-cli"),
            "command": creds.get("command", "agy"),
            "args": list(creds.get("args") or []),
            "source": "google-oauth-via-antigravity-cli",
            "requested_provider": requested_provider,
        }
    raise AuthError(
        f"Provider {provider!r} is disabled by Bithumb Agent' OAuth-only policy.",
        code="bithumb_agent_oauth_only",
    )
