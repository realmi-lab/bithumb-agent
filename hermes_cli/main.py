#!/usr/bin/env python3
"""
Bithumb Agent CLI - Hermes-compatible main entry point.
...
def _set_process_title() -> None:
    """Set the process title to 'bithumb-agent' so tools like 'ps', 'top', and
    ...
    try:
        ...
        setproctitle.setproctitle("bithumb-agent")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"bithumb-agent", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"bithumb-agent")
...
def _print_fast_version_info() -> None:
    ...
    print(f"Bithumb Agent v{__version__} ({__release_date__}) · OSS customization")
    print("Runtime: customized from Hermes Agent (MIT) · Antigravity-inspired UX")
...
def cmd_update(args):
    ...
    """
    if os.environ.get("BITHUMB_AGENT_DISABLE_AUTO_UPDATE") == "1":
        print("Bithumb Agent self-update is disabled. Review and deploy a pinned build manually.")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("B Updating Bithumb Agent (Hermes upstream runtime)...")
    print()
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        ...
        return ["auto", "openai-codex", "antigravity-cli"]
...
def main():
    ...
    from hermes_cli.bithumb_agent_policy import cli_command_is_allowed

    if not cli_command_is_allowed(getattr(args, "command", None)):
        print(
            f"Command {getattr(args, 'command', None)!r} is disabled by the "
            "Bithumb Agent coding-only security policy.",
            file=sys.stderr,
        )
        return
