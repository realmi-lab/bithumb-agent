...
_PROVIDER_MODELS: dict[str, list[str]] = {
    ...
    # The official Antigravity CLI owns model discovery and may rotate its
    # default without a Bithumb Agent release. "auto" intentionally omits --model.
    "antigravity-cli": [
        "auto",
    ],
    "zai": [
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry(
        "antigravity-cli",
        "Gemini OAuth",
        "Gemini via Antigravity OAuth (baseline quota only; no API key or paid overage)",
    ),
]
...
# Bithumb Agent accepts friendly names only for its two OAuth-backed runtimes.
_PROVIDER_ALIASES = {
...
def _provider_keys(provider: str) -> set[str]:
    ...
    try:
        ...
        # Keep static-catalog helpers compatible with upstream provider data.
        # This does not enable the provider: Bithumb Agent' auth/runtime boundaries
        # still reject every provider outside the two OAuth routes.
...
def detect_static_provider_for_model(
    ...
    if resolved_provider not in {"custom", "openrouter"}:
        ...
        if (
            resolved_provider in _PROVIDER_MODELS
            and default_models
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    """
    from hermes_cli.bithumb_agent_policy import normalize_provider as _normalize_bithumb_agent_provider

    return _normalize_bithumb_agent_provider(provider, allow_auto=True)
