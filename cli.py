#!/usr/bin/env python3
"""
Bithumb Agent CLI - Open-source Interactive Terminal Interface

An orange-branded downstream customization of Hermes Agent (MIT), with an
Antigravity-inspired startup experience, interactive REPL, tools, and skills.
...
# Backward-compatible aliases for the customized visual brand.
from hermes_cli.banner import BITHUMB_AGENT_LOGO, BITHUMB_MONOGRAM
HERMES_AGENT_LOGO = BITHUMB_AGENT_LOGO
HERMES_CADUCEUS = BITHUMB_MONOGRAM
...
def _build_compact_banner() -> str:
    ...
    border_color = _skin.get_color("banner_border", "#CF4F08") if _skin else "#CF4F08"
    title_color = _skin.get_color("banner_title", "#FF8A3D") if _skin else "#FF8A3D"
    dim_color = _skin.get_color("banner_dim", "#A54100") if _skin else "#A54100"
    text_color = _skin.get_color("banner_text", "#FFF3EB") if _skin else "#FFF3EB"

    if skin_name == "default":
        line1 = "BITHUMB AGENT · OPEN-SOURCE AI AGENT"
        tiny_line = "BITHUMB AGENT"
    else:
    ...
    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        ...
        version_line = f"Bithumb Agent v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]· Hermes Agent (MIT) customization[/]\n"
    ...
    line3 = "오픈소스를 빗썸에 맞게 바꾼 것입니다. · ilhong.kim@bithumbcorp.com"[:content_width].ljust(content_width)
    line4 = "Hermes Agent (MIT) customization · Antigravity-inspired UX"[:content_width].ljust(content_width)

    return (
        f"\n[bold {border_color}]╔{bar}╗[/]\n"
        f"[bold {border_color}]║[/] [{title_color}]{line1}[/] [bold {border_color}]║[/]\n"
        f"[bold {border_color}]║[/] [dim {dim_color}]{line2}[/] [bold {border_color}]║[/]\n"
        f"[bold {border_color}]║[/] [bold {text_color}]{line3}[/] [bold {border_color}]║[/]\n"
        f"[bold {border_color}]║[/] [dim {dim_color}]{line4}[/] [bold {border_color}]║[/]\n"
...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin, CLIBillingMixin):
    """
    Interactive CLI for Bithumb Agent's Hermes-derived runtime.
    ...
    def __init__(
        ...
        """
        Initialize the Bithumb Agent CLI.
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Bithumb Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Bithumb Agent! Type your message or /help for commands."
...
def main(
    ...
    """
    Bithumb Agent CLI - Open-source interactive AI assistant
